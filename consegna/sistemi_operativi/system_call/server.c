...
void remove_ipcs(){
    ...
    close(fifo1_fd);                        // chiusura e rimozione fifo
    ...
    free_shared_memory(shm_buffer);         // detach shared memory
    free_shared_memory(shm_flags);

    remove_shared_memory(shmid);            // rimozione shared memory
    remove_shared_memory(shm_flags_id);

    msgctl(msg_queue_id, IPC_RMID, NULL);   // chiusura msg queue

    if (semctl(semid, 0 /*ignored*/, IPC_RMID, NULL) == -1)     // chiusura set semafori
        ErrExit("semctl IPC_RMID semid failed");
...
int main(int argc, char * argv[]) {
    ...
    unlink(FIFO1_NAME);         // rimozione preventiva delle fifo
    unlink(FIFO2_NAME);         // utile solo per l'avvio dopo un eventuale crash del programma
    ...
    while(true){
        ...
        errno = 0;
        while(read(fifo1_fd, &N, sizeof(int)) != sizeof(int)){
            if(errno != 0)
                ErrExit("read fifo1 failed");
        }
